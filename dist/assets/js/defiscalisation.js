...
$(document).ready(function () {
  ...
  var isChangingContent = false;
  var percent = 75; // Capture scroll events
  ...
  $(".don-type-item").on("click", function () {
    if (isChangingContent === true) return;
    ...
    var oldElem = $(".don-type-content.active");
    ...
    var newAmounts = deductionValues[target]['data'];
    var newImage = deductionValues[target]['image'];
  ...
  function freeAmountHandler() {
    ...
    var leftBottomAmount = parseFloat(amount * percent / 100);
    if (leftBottomAmount > 50000) leftBottomAmount = 50000;
    var topRightAmount = amount - leftBottomAmount;
    if (isNaN(topRightAmount) || isNaN(leftBottomAmount)) return;
    isInt(leftBottomAmount) ? $(".circle-right .amount-left-bottom").text(leftBottomAmount + "€") : $(".circle-right .amount-left-bottom").text(leftBottomAmount.toFixed(2) + "€");
    isInt(topRightAmount) ? $(".circle-right .amount-right-top").text(topRightAmount + "€") : $(".circle-right .amount-right-top").text(topRightAmount.toFixed(2) + "€");
  }

  $(".free-amount").on("input", freeAmountHandler);

  function modifyCircleValue(circle, amounts, newImage) {
    $(circle).find('.circle-background').css('background-image', newImage);
  ...
  function modifyCircleValues(amounts, newImage) {
    ...
    $('.circle-right').find('.circle-background').css('background-image', newImage);
    $('.circle-right').find('.free-amount').val(amounts[2]['amount-center']).trigger('input'); //modifyCircleValue('.circle-right', amounts[2]);
  }
});